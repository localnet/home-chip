 ...
 * @home-chip/logger. Most tests only need somewhere for the lines to go; `calls` is there for the
 * one that checks a migration did not run again.
 *
 * `get()` returns the same instance: this package writes under one facility, and composing them is
 * the logger package's concern.
 */
