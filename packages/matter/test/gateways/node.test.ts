...
describe("SdkNodeGateway", () => {
    describe("commission", () => {
        test("joins a device and returns its node and endpoints", async (t) => {
    ...
    describe("decommission", () => {
        test("removes a node from the fabric and forgets it", async (t) => {
            const { gateway, pairingCode } = await setup(t);
            const result = await gateway.commission(pairingCode);
            await gateway.decommission(result.node.id);
            assert.throws(() => gateway.getInfo(result.node.id), NodeNotFoundError);
        });
    ...
    describe("getInfo", () => {
        test("returns the device's basic information", async (t) => {
        ...
        test("throws NodeNotFoundError for a node it does not hold", async (t) => {
            const { gateway } = await setup(t);
            assert.throws(() => gateway.getInfo("00000000-0000-7000-8000-000000000000" as never), NodeNotFoundError);
        });
    });

    describe("isReachable", () => {
        test("is true for a commissioned, online node", async (t) => {
            const { gateway, pairingCode } = await setup(t);
            const result = await gateway.commission(pairingCode);
            assert.equal(gateway.isReachable(result.node.id), true);
        });

        test("is false, rather than throwing, for a node it does not hold", async (t) => {
            const { gateway } = await setup(t);
